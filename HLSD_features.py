...
def save_features():
	...
	#アルファベット名のoutput内のファイル名を収集
	parsed_event = 0
	empty_event = 0
	error_json = 0

	for categ in categs:#全てのアルファベット名に対して			
		pieces = sorted(glob(os.path.join(categ, "*{}".format(sep))))
		#アーティスト名を収集
		for piece in pieces:#全てのアーティスト名に対して
			songs = sorted(glob(os.path.join(piece, "*{}".format(sep))))
			#曲名を収集
			for song in songs:#全ての曲名ファイルに対して
				parts = sorted(glob(os.path.join(song, "*.json")))
				for part in parts:#全てのjsonファイルに対して
					...
					part_name = os.path.basename(".".join(part.split(".")[:-1]))#part name

					try:
						input_list = parse_HLSD_features(part)#jsonファイルを入力
						#melody,chord,[mode,key]を返す
...
def parse_HLSD_features(json_file):
	...
	- すべて4/4 
	- キー メジャー/マイナー/その他
		- その他をM/mのいずれかに正規化する必要があります
	- 記号、ルート、タイプ、モードを追加で取得する必要がある  
	'''
	# load json読み込むのはoutputのファイル
	with open(json_file, "r") as st_json:
		info = json.load(st_json)#jsonファイルを辞書として読み込み

	orig_parent_path = orig_dirname#元のeventの場所
	part_path = sep.join(json_file.split(sep)[4:])#partのパス
	orig_json_file = os.path.join(orig_parent_path, part_path)#元のデータパス
	assert os.path.exists(orig_json_file)#元のパスが存在するかどうか
	...
	if len(orig_melody) > 0 and len(orig_chord) > 0:
		...
		#半小節ごとに2つの和音をサンプリングするために、和音を再分割する
		'''
		...
		new_chord_list = list()
		prev_start = 0
		for each_chord in orig_chord:#オリジナルの全てのコードデータに対して
			# original info 
			...
			if start % 2 == 0 and dur <= 2:
				...
				#startが2で割り切れて且つdurが2より大きい場合はendをstart+2
				elif start % 2 > 0: # no matter the duration
					new_end = (start//2)*2 + 2 # new_end - new_start < 2
				#startを2で割ると、0より大きくなる場合は
				#endを2でわった余りの半小節分も加えて終わる
				while new_end < end:#newendがオリジナルデータのendよりも小さいとき
					new_chord = copy.deepcopy(each_chord) # copy original chord
					new_chord['event_on'] = new_start #new_startを更新
					new_chord['event_off'] = new_end #new_endを更新
					# print(new_start, new_end)
					new_chord_list.append(new_chord)
					new_start = new_end  #new_startを更新
					new_end = new_start+2#new_endを更新
				...
				# print("--------------------")
				new_chord_list.append(new_chord)
				#結局半小節分に細分化したコード進行を時間的な被り関係なく入れていく
		...
		beat_sum = 0
		for i in range(0, int(song_end), 2):#曲の終わりまで半小節毎の時間で
			half_start, half_end = i, i+2
			half_measure = list()
			# print("------------")
			# print(i, i+2)
			for each_chord in new_chord_list:#全ての半小節時間でのコードに対して
				start, end = each_chord['event_on'], each_chord['event_off']
				dur = each_chord['event_duration']
				if start >= half_start and end <= half_end:
					#startが半小節の開始以上、endが半小節の終わり以下
					# print(start, end)
					half_measure.append(each_chord)
			if len(half_measure) == 0:#もしhalf_measureが0なら
		...
		for mm in half_measure_dict:
			...
			'''
			half_start, half_end = mm.split("_")#半小節の開始と終わりの宣言
			chord_hist = dict()
			each_half = half_measure_dict[mm]
			for h in each_half:#半小節ないの全てのコードに対して
				chord_name = '_'.join(
	...
	return info#melodyと新たなchordと新たなmodeとkeyデータ

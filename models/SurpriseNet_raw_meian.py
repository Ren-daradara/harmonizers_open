...
class CVAE(nn.Module):
    def __init__(self,
        ...
        if self.model_type == 'CVAE':
            ...
            self.lands_prenet = nn.LSTM(input_size=1, 
                                   hidden_size = self.params_prenet_size , 
                                   num_layers=self.params_prenet_num_layers,
                                   batch_first=True, 
                                   dropout=0.2,
        ...
        # Encoder
        self.encoder = nn.LSTM(input_size=self.params_num_chord + self.BEAT_RESOLUTION * 25 + self.params_prenet_size * 2+1, 
        ...
        # Latent to decoder
        self.latent2decoder_input = nn.Linear(self.params_latent_size + self.BEAT_RESOLUTION * 25 + self.params_prenet_size * 2+1, self.params_decoder_hidden_size // 2,device=self.device)
...
#         print('length',length.shape)
    ...
    """
    def lands_embedding(self, lands_condition):
        
#         print('surprise',surprise_condition.shape)
#         print('length',length.shape)
        # Pack data to encoder
        #packed_x = pack_padded_sequence(surprise_condition, length, batch_first=True, enforce_sorted=False)
        lands_output , (hidden, _) = self.lands_prenet(lands_condition)
        # Pad back
        #prenet_output, _ = pad_packed_sequence(prenet_output, batch_first=True, total_length=self.max_sequence_length)
...
#         print('length',length.shape)
    ...
    def forward(self, input_chord,length,melody_condition,surprise_condition,lands_condition):
        ...
        if self.model_type == 'SurpriseNet':
            ...
            encoder_input = torch.cat([input_chord,melody_condition,surprise_condition,lands_condition], dim = -1)
            decoder_input = torch.cat([melody_condition, surprise_condition,lands_condition],dim = -1)
    ...
    def test(self, length,melody_condition,surprise_condition,z,lands_condition):
        surprise_condition = self.surprise_embedding(surprise_condition,length)
        #lands_condition = self.lands_embedding(lands_condition)

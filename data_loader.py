...
class HLSDDataset(Dataset):
    def __init__(self, 
                 ...
                 y=None,
                 device=None):
        #mangaerは他のプログラミングがオブジェクトを操作できる様にする
        manager = Manager()
        self.x = manager.list(x)
        self.k = manager.list(k)
        self.n = manager.list(n)
...
class HLSDPadCollate(object):
    ...
    def pad_collate(self, batch):
        ...
        ys = [torch.from_numpy(b[4]) for b in batch]
        cs = [torch.from_numpy(b[5]) for b in batch]
        modes = [torch.from_numpy(b[6]) for b in batch]#TensorからNumpy_ndarrayに変換
        ...
        ks = torch.stack(list(ks), dim=0)
        cs = torch.stack(list(cs), dim=0)
        modes = torch.stack(list(modes), dim=0)
        return xs, ks, ns, ms, ys, cs, modes
...
class LSTMDataset(Dataset):
    def __init__(self, 
        ...
        self.m = manager.list(m)
        self.y = manager.list(y)
        self.z = manager.list(z)
        self.s = manager.list(s)
        self.device = device
    ...
    def __getitem__(self, idx):
        ...
        m = np.load(self.m[idx]).astype(np.float32)
        y = np.load(self.y[idx]).astype(np.float32)
        z = np.load(self.z[idx]).astype(np.float32)
        s = np.load(self.s[idx]).astype(np.float32)
        x_ = np.argmax(x, axis=-1)
        y_ = np.argmax(y, axis=-1) 
        k_ = np.asarray(12 * k)
        ...
        return x_, k_, n, m, y_, clab_,k,z,s

class LSTMPadCollate(object):
    '''
    Ref: https://discuss.pytorch.org/t/dataloader-for-various-length-of-data/6418/7
    '''
    ...
    def pad_collate(self, batch):
        ...
        cs = [torch.from_numpy(b[5]) for b in batch]
        modes = [torch.from_numpy(b[6]) for b in batch]#TensorからNumpy_ndarrayに変換
        zs = [torch.from_numpy(b[7]) for b in batch]
        ss = [torch.from_numpy(b[8]) for b in batch]
        ...
        cs = torch.stack(list(cs), dim=0)
        modes = torch.stack(list(modes), dim=0)
        zs = torch.stack(list(zs), dim=0)
        ss = torch.stack(list(ss), dim=0)
        return xs, ks, ns, ms, ys, cs, modes, zs, ss
